...
std::vector<int> findOccurences(std::vector<seqan3::dna5> const& ref, std::vector<seqan3::dna5> const& query) {
    //!TODO ImplementMe
    vector<int> occurenceIndices;
    for(int i =0; i<ref.size()-query.size()+1;i++){
        bool flag = true;
        ...
        if(flag){
            occurenceIndices.push_back(i);
        }
    }
    return occurenceIndices;
}
